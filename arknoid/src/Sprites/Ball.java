...
public class Ball implements Sprite, HitNotifier {
    ...
    public void moveOneStep() {
        ...
        if (allCollidables.getClosestCollision(trajectory) == null) {
            ...
            if ((center.getX() > collidingRect.getUpperLeft().getX())
                ...
                this.center = this.getVelocity().applyToPoint(new Point(collisionPoint.getX(),
                        center.getY() - 4 * this.getSize()));
                //this.center = this.getVelocity().applyToPoint(new Point(center.getX(),
                        //center.getY() - 2 * this.getSize()));
            }
            this.setVelocity(collisionObject.hit(this, collisionPoint, ballVelocity));
        }
    }
    /**
    ...
    /**
     * This method is in charge of removing a ball from the game.
     * @param game is the given running game.
     */
    public void removeFromGame(GameLevel game) {
        game.removeSprite(this);
        int currNumOfBalls = game.getNumOfBalls().getValue();
        game.setNumOfBalls(currNumOfBalls - 1);
    }
}