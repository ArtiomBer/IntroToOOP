...
public class Line {
    ...
    /**
     * This method checks if a there is an intersection point
     * with a rectangle and returnes the closest one to the start
     * of a line.
    ...
    public Point closestIntersectionToStartOfLine(Rectangle rect) {
        Line testLine = this;
        List<Point> intersectionList = rect.intersectionPoints(testLine);
        if (intersectionList.isEmpty()) {
            return null;
        }
        Point closestIntersection = intersectionList.get(0);
        double minLength = testLine.start.distance(closestIntersection);
        for (Point p: intersectionList) {
            if (p.distance(testLine.start) < minLength) {
                minLength = p.distance(testLine.start);
                closestIntersection = new Point(p.getX(), p.getY());
            }
        }
